...
flags.DEFINE_string("data_dir", './data/ner', "The input datadir.", )
flags.DEFINE_string("bert_config_file", './model/bert_config.json',
                    "The config json file corresponding to the pre-trained BERT model.")
flags.DEFINE_string("task_name", 'ner', "The name of the task to train.")
flags.DEFINE_string("output_dir", './output/ner', "The output directory where the model checkpoints will be written.")
...
label_list = ["B-MISC", "I-MISC", "O", "B-PER", "I-PER", "B-ORG", "I-ORG", "B-LOC", "I-LOC", "X", "[CLS]", "[SEP]"]
...
class NerProcessor():
    ...
    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "valid.txt")), "valid"
        )
        ...
        with codecs.open(input_file, "r", "utf-8") as f:
            ...
            for line in f:
                ...
                if len(contends) == 0 and len(labels) > 0:
                    ...
                    labels = ''
                    words = ''
                elif len(label) > 0 and len(word) > 0:
                    labels += label + ' '
                    words += word + ' '
                continue
            return lines
...
def convert_single_example(example, max_seq_length, tokenizer):
    ...
    label_ids = [label_map["[CLS]"]] + [label_map[k] for k in labels] + [label_map["[SEP]"]]
...
def filed_based_convert_examples_to_features(examples, max_seq_length, tokenizer, output_file, mode=None):
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        filed_based_convert_examples_to_features(train_examples, FLAGS.max_seq_length, tokenizer, train_file)
    ...
    if FLAGS.do_eval:
        ...
        filed_based_convert_examples_to_features(eval_examples, FLAGS.max_seq_length, tokenizer, eval_file)
